...
if (args.equipped) {
  ...
  this.script.message(`Punkty Szczescia wzrosly z ${currentFortune} do ${runeFortune + currentFortune}.`)
}
else
{
  this.item.flags.runeOfLuck = true
  await this.actor.update({"system.status.fortune.value" : Math.max(0, currentFortune - runeFortune)})
  this.script.message(`Punkty Szczescia zmniejszone do ${Math.max(0, currentFortune - runeFortune)}.`)
}
