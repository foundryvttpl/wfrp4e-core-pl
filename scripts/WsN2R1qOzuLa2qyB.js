...
let rune;
let categories = [];
if (specifier && specifier.toLowerCase() != "wszystkie formy")
{
  if (specifier.includes("Talizman"))
  {
    categories.push("talisman");
  }
  if (specifier.includes("Ochrona"))
  {
    categories.push("protection");
  }
  if (specifier.includes("Bron"))
  {
    categories.push("weapon");
  }
  if (specifier.includes("Pancerz"))
  {
  ...
  if (categories.length)
  {
    ...
    {
      rune = await DragDialog.create({text : `Provide Rune to learn (${specifier})`, title : this.effect.name, filter: (item) => item.type == "wfrp4e-dwarfs.rune" && !item.system.master, onError: "Must provide a Rune (non-Master)"});
    }
  }
  else 
  {
...
{
  rune = await DragDialog.create({text : `Provide Rune to learn`, title : this.effect.name, filter: (item) => item.type == "wfrp4e-dwarfs.rune" && !item.system.master, onError: "Must provide a Rune (non-Master)"});
}
